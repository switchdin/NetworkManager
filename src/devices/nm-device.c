                    ...
                    GHashTable *diffs, GError **error)
{
	...
	g_return_val_if_fail (diffs, FALSE);

	if (nm_streq (setting_name, NM_SETTING_CONNECTION_SETTING_NAME)) {
		/* Whitelist allowed properties which are allowed to differ.
		...
		if (!nm_device_hash_check_invalid_keys (diffs,
			...
			return FALSE;
	} else if (   nm_streq (setting_name, NM_SETTING_IP4_CONFIG_SETTING_NAME)
	           || nm_streq (setting_name, NM_SETTING_IP6_CONFIG_SETTING_NAME)) {
		/* Accept all changes */
	} else {
		g_set_error (error,
		             NM_DEVICE_ERROR,
...
{
	/* Stub implementation for reapply_connection(). It does nothing,
	 * but allows derived classes to uniformly invoke the parent
	 * implementation. */
}
