...
{
	NM80211WpsFlags flags = NM_802_11_AP_WPS_DISABLED;
	const char *tmp;
	...
	if (g_variant_lookup (wps, "Type", "&s", &tmp)) {
		if (strcmp (tmp, "") == 0)
			flags = NM_802_11_AP_WPS_AUTO;
		else if (strcmp (tmp, "pbc") == 0)
			flags = NM_802_11_AP_WPS_PBC;
		else if (strcmp (tmp, "pin") == 0)
			flags = NM_802_11_AP_WPS_PIN;
		else
			flags = NM_802_11_AP_WPS_UNKNOWN;
	}
              ...
              GValue *value, GParamSpec *pspec)
{
	...
	case PROP_RSN_FLAGS:
		...
		break;
	case PROP_WPS_FLAGS:
		g_value_set_uint (value, priv->wps_flags);
		break;
	case PROP_SSID:
		if (priv->ssid) {
...
nm_wifi_ap_init (NMWifiAP *ap)
{
	...
	priv->wps_flags = NM_802_11_AP_WPS_AUTO;
	priv->last_seen = -1;
}
...
#define ALL_SEC_FLAGS \
	...
	obj_properties[PROP_WPS_FLAGS] =
	    g_param_spec_uint (NM_WIFI_AP_WPS_FLAGS, "", "",
