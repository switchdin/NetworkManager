...
typedef struct {
	NMSupplicantInterface *self;
	NM80211WpsFlags flags;
	char *bssid;
	char *pin;
} WpsEnrollData;

static void
wps_enroll_data_free (WpsEnrollData *data)
{
	g_object_unref (data->self);
	g_free (data->pin);
	g_free (data->bssid);
	g_slice_free (WpsEnrollData, data);
}
...
on_wps_proxy_acquired (GDBusProxy *proxy, GAsyncResult *result, gpointer user_data)
{
	...
	g_variant_builder_add (&start_args, "{sv}", "Type", g_variant_new_string (type));
	...
	_LOGI ("starting '%s' WPS enrollment for BSSID '%s'", type, data->bssid);
	g_dbus_proxy_call (priv->wps_proxy,
	                   "Start",
                                    ...
                                    const char *pin)
{
	...
	data = g_slice_new0 (WpsEnrollData);
